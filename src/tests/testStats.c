...
void test_ST_statistiques(void)
{
  ...
  for (long int i = 0; i < 256; i++)
    CU_ASSERT_TRUE(stats.lesStatistiques[i] == 0);
}
...
int main(int argc, char **argv)
{
  ...
  if ((NULL == CU_add_test(pSuite, "ST_statistiques", test_ST_statistiques)) || (NULL == CU_add_test(pSuite, "ST_obtenirOccurenceOctet", test_ST_obtenirOccurenceOctet)) || (NULL == CU_add_test(pSuite, "ST_incrementerOccurenceOctet", test_ST_incrementerOccurenceOctet)))
  {
    CU_cleanup_registry();
    return CU_get_error();
  }
